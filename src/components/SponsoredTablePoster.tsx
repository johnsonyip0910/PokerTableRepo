...
interface SponsoredTablePosterProps {
  table: TableRecord;
...
export function SponsoredTablePoster({ 
  table, 
  onClick, 
...
}: SponsoredTablePosterProps) {
  const getCardClassName = () => {
    let baseClasses = "bg-card border-border cursor-pointer transition-all duration-200 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 overflow-hidden relative";
  ...
  return (
    ...
    >
      ...
      {sponsorBackgroundImage && (
        <div className="absolute inset-0 z-0">
          <ImageWithFallback
            ...
            alt={`${table.name} sponsored background`}
          ...
          <div 
            ...
            style={{ 
              opacity: sponsorOverlayOpacity
            }}
          />
        </div>
      )}

      {/* Subtle Sponsored Label - Top Right Corner */}
      <div className="absolute top-3 right-3 z-20">
        <span className="text-xs text-muted-foreground/60 bg-card/80 px-2 py-1 rounded backdrop-blur-sm">
          Sponsored
        </span>
      </div>
      ...
      <CardContent className="p-4 relative z-10">
        ...
        {/* Action button - visually identical to normal TableCard */}
