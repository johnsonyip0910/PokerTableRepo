...
// Convert TableRecord to legacy TableRow format for display compatibility with safe date parsing
export function tableRecordToTableRow(record: TableRecord): TableRow {
  // Safely parse and format the start time
  let formattedStartTime = 'TBD';
  try {
    const startDate = new Date(record.startTimeISO);
    if (!isNaN(startDate.getTime())) {
      ...
      console.warn('Invalid startTimeISO for table:', record.id, record.startTimeISO);
    }
  } catch (error) {
    console.warn('Error parsing startTimeISO for table:', record.id, error);
  }
...
export function sortTables(tables: TableRecord[], sortBy: TableFilters['sortBy']): TableRecord[] {
  ...
  switch (sortBy) {
    ...
    case 'startTime':
      return sorted.sort((a, b) => {
        try {
          const dateA = new Date(a.startTimeISO);
          const dateB = new Date(b.startTimeISO);
          ...
          console.warn('Error sorting by startTime:', error);
          return 0;
        }
      });
...
export function generateDayScheduleTables(date: Date): TableRecord[] {
  ...
  if (!date || isNaN(date.getTime())) {
    console.warn('Invalid date provided to generateDayScheduleTables:', date);
    return [];
  }
  ...
  for (let i = 0; i < 10; i++) {
    ...
    // Create start time for this slot with validation
    const startDateTime = new Date(date.getTime()); // Create a copy of the date
    startDateTime.setHours(timeSlot.hour, timeSlot.minute, 0, 0);
    
    // Validate the created datetime
    if (isNaN(startDateTime.getTime())) {
      console.warn('Invalid datetime created for slot:', timeSlot, 'with date:', date);
      continue; // Skip this table if datetime is invalid
    }
...
export function generateTablesForDate(date: Date, count: number = 100): TableRecord[] {
  // Validate input date
  if (!date || isNaN(date.getTime())) {
    console.warn('Invalid date provided to generateTablesForDate:', date);
    return [];
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Create ISO time string for the given date with validation
    const startDateTime = new Date(date.getTime()); // Create a copy of the date
    startDateTime.setHours(baseHour, minutes, 0, 0);
    
    // Validate the created datetime
    if (isNaN(startDateTime.getTime())) {
      console.warn('Invalid datetime created with baseHour:', baseHour, 'minutes:', minutes, 'date:', date);
      continue; // Skip this table if datetime is invalid
    }
